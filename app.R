...
ui <- fluidPage(
    ...
    titlePanel("Shiny Project"),
        navbarPage("",id="tabx",
                 tabPanel("Trains dataset",
                            DT::dataTableOutput("Trains_data")
                 ),
                 tabPanel("Trains Summary",
                          verbatimTextOutput("summary_trains")
                 ),
                 tabPanel("Trains plot",
                        sidebarLayout(
                            sidebarPanel(
                ...
                tabPanel("Planes dataset",
                         DT::dataTableOutput("Planes_data")
                ),
                tabPanel("Planes Summary",
                         verbatimTextOutput("summary_planes")
                ),
               
                tabPanel("Planes plot",
                        sidebarLayout(
                            sidebarPanel(
                                selectInput("ylabel1planes", "label for first graph (sums):",
                                            choices= list("DISTANCE","DELAYED","TOTAL")),
                ...
                tabPanel("Flight map",plotlyOutput("plotlymap") )
        )
...
server <- function(input, output) {
    ...
    output$Trains_data <- DT::renderDataTable({
        Trainsdata <- df[1:100, -which(names(df) %in% c("comment_cancellations","comment_delays_at_departure","comment_delays_on_arrival"))]
        DT::datatable(Trainsdata)
    })
    
    output$Planes_data <- DT::renderDataTable({
        Planesdata <- dfused[1:100,]
        # -which(names(df) %in% c("comment_cancellations","comment_delays_at_departure","comment_delays_on_arrival"))
        DT::datatable(Planesdata)
    })
    
    output$summary_trains <- renderPrint({
        summary(df[,-which(names(df) %in% c("comment_cancellations","comment_delays_at_departure","comment_delays_on_arrival"))])
    })
    
    output$summary_planes <- renderPrint({
        summary(dfused)
    })
